...
document.addEventListener('DOMContentLoaded', () => {
  ...
  resetBtn.addEventListener('click', () => {
    ...
    /*
     * Mobile swipe navigation: detect vertical swipe gestures on the
     * reader section to navigate between cards. A swipe upwards moves
     * forward, while a swipe downwards moves back. We record the touch
     * start position and current position during the gesture and only
     * trigger navigation for quick, substantial movements (over 50px
     * within 700ms). Touch listeners are attached to the reader rather
     * than the card container to avoid conflicts with scrolling inside
     * the card.
     */
    ...
    reader.addEventListener('touchmove', e => {
      if (swipeStartY !== null && e.touches && e.touches.length === 1) {
    ...
    reader.addEventListener('touchend', e => {
      ...
      if (Math.abs(diffY) > 50 && duration < 700) {
        if (diffY < 0) {
          nextCard();
        } else {
          prevCard();
